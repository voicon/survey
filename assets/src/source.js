...
$(function(){
    ...
    $("#btnGridSave").click(function(){
        jQuery.ajax({
            type: 'POST',
            url: "/" + BN + "/src/main.php",
            data: {a: 'save', f: jQuery('#filename').val(), data: JSON.stringify($("#treegrid").fancytree("getTree").toDict())},
    ...
    $("#btnGridLoad").click(function(){
        initTree(jQuery('#filename').val());
    });
});
function initTree(sFile) {
    var sFilePath = "/" + BN + "/data/" + sFile;
    $("#tree").fancytree({
